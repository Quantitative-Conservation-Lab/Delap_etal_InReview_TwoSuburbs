...
for(s in 1:n.sites) {
  for(y in 1:n.years){
    for(k in 1:n.species){
      ...
      #model on detection
      logit(p[s,y,k]) <- int.p + p.ran[s,y] + p.ran.species[k]
      #model on occupancy 
...
params <- c("int.p","p.site.type","sd.p","sd.p.species","int.psi","beta.site.type","beta.year","beta.year2","sd.psi.species","w.sitetype","w.year","w.year2") 
