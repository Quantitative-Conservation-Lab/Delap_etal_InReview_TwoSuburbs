...
results.mat <- matrix(NA,nrow=Species,ncol = 110)
...
for(s in 1:n.sites){
  for(y in 1:n.years){
    for(k in 1:n.species){
      ...
      logit(psi[s,y,k]) <- int.psi + best.site.type[site.type[s]] + psi.ran.species[k]
    }#k species 
  }#y year
}#j site 
  ...
  for(s in 1:2){
    for(y in 1:n.years){
      for(k in 1:n.species){
        psi.pred[s,y,k] <- 1/(1+exp(-(int.psi + best.site.type[site.type[s]] + psi.ran.species[k])))
      }  
    }
  }
...
for(k in 1:n.species){
  p.ran.species[k] ~ dnorm(0,sd = sd.species)
...
params <- c("psi.pred","int.psi","beta.site.type","psi.ran.species")
