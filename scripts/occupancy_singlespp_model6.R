...
for(s in 1:n.sites){
  for(y in 1:n.years){
    ...
    logit(psi[s,y]) <- int.psi + beta.site.type[site.type[s]] + beta.year*y + beta.year2*pow(y,2)
  }
}
  
for(s in 1:2){
  for(y in 1:n.years){
    psi.pred[s,y] <- 1/(1+exp(-(int.psi + beta.site.type[site.type[s]] + beta.year*y + beta.year2*pow(y,2))))
  }
} 
