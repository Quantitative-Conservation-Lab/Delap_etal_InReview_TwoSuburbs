...
for(s in 1:n.sites){
  for(y in 1:n.years){
    ...
    logit(psi[s,y]) <- int.psi + beta.year*year.norm[y] + beta.year2*year2.norm[y]
  }
}
  
for(s in 1:2){
  for(y in 1:n.years){
    psi.pred[s,y] <- 1/(1+exp(-(int.psi + beta.year*year.norm[y] + beta.year2*year2.norm[y])))
  }
} 
...
params <- c("psi.pred","int.p","int.psi","beta.year","beta.year2")
...
params <- c("psi.pred[1, 1]","psi.pred[1, 2]","psi.pred[1, 3]","psi.pred[1, 4]","psi.pred[1, 5]","psi.pred[1, 6]","psi.pred[1, 7]","psi.pred[1, 8]","psi.pred[1, 9]","psi.pred[1, 10]","psi.pred[1, 11]","psi.pred[1, 12]",
            ...
            "int.p","int.psi","beta.year","beta.year2") 
...
#R-hat 
results.mat[i,114] <- gelman.diag(out$samples[,c(1:4)],multivariate=TRUE)$mpsrf
...
write.csv(results.mat,"results/occ_model4_results.csv")
write.csv(all.species,"results/occ_model4_allspp.csv")
