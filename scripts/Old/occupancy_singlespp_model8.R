...
results.mat <- matrix(NA,nrow=length(Species),ncol = 126)
...
for(s in 1:n.sites){
  for(y in 1:n.years){
    ...
    logit(psi[s,y]) <- int.psi + beta.site.type[site.type[s]] + beta.year*year.norm[y] + beta.year2*year2.norm[y] + beta.site.type.year[site.type[s]]*year.norm[y] + beta.site.type.year2[site.type[s]]*year2.norm[y]
  }
}
  
for(s in 1:2){
  for(y in 1:n.years){
    psi.pred[s,y] <- 1/(1+exp(-(int.psi + beta.site.type[site.type[s]] + beta.year*year.norm[y] + beta.year2*year2.norm[y] + beta.site.type.year[site.type[s]]*year.norm[y] + beta.site.type.year2[site.type[s]]*year2.norm[y])))
  }
} 
...
beta.site.type.year[1] <- 0 
beta.site.type.year[2] ~ dnorm(0,sd=1)
beta.site.type.year2[1] <- 0
beta.site.type.year2[2] ~ dnorm(0,sd=1)
...
params <- c("psi.pred","int.p","int.psi","beta.site.type","beta.year","beta.year2","beta.site.type.year","beta.site.type.year2")
...
params <- c("psi.pred[1, 1]","psi.pred[1, 2]","psi.pred[1, 3]","psi.pred[1, 4]","psi.pred[1, 5]","psi.pred[1, 6]","psi.pred[1, 7]","psi.pred[1, 8]","psi.pred[1, 9]","psi.pred[1, 10]","psi.pred[1, 11]","psi.pred[1, 12]",
            ...
            "int.p","int.psi","beta.site.type[2]","beta.year","beta.year2","beta.site.type.year[2]","beta.site.type.year2[2]") 
...
#coefficients mean
results.mat[i,97:103] <- apply(out.all[,25:31], 2, mean)  
#coefficients sd 
results.mat[i,104:110] <- apply(out.all[,25:31], 2, sd)
#coefficients LCI 
results.mat[i,111:117] <- apply(out.all[,25:31], 2, function(x)quantile(x,probs=0.025)) 
#coefficients UCI 
results.mat[i,118:124] <- apply(out.all[,25:31], 2, function(x)quantile(x,probs=0.975)) 

#WAIC
results.mat[i,125] <- out$WAIC$WAIC
#R-hat 
results.mat[i,126] <- gelman.diag(out$samples[,c(2,4,6,7,8,9,10)],multivariate=TRUE)$mpsrf
...
colnames(results.mat) <- c(paste(params[1:24],".mean",sep=""),paste(params[1:24],".sd",sep=""),paste(params[1:24],".LCI",sep=""),paste(params[1:24],".UCI",sep=""),paste(params[25:31],".mean",sep=""),paste(params[25:31],".sd",sep=""),paste(params[25:31],".LCI",sep=""),paste(params[25:31],".UCI",sep=""),"WAIC","R.hat")

all.species <- apply(z.pred,c(2,3,4),sum)


write.csv(results.mat,"results/occ_model8_results.csv")
write.csv(all.species,"results/occ_model8_allspp.csv")
