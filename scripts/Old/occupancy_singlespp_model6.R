...
results.mat <- matrix(NA,nrow=length(Species),ncol = 118)
...
for(s in 1:n.sites){
  for(y in 1:n.years){
    ...
    logit(psi[s,y]) <- int.psi + beta.site.type[site.type[s]] + beta.year*year.norm[y] + beta.year2*year2.norm[y]
  }
}
  
for(s in 1:2){
  for(y in 1:n.years){
    psi.pred[s,y] <- 1/(1+exp(-(int.psi + beta.site.type[site.type[s]] + beta.year*year.norm[y] + beta.year2*year2.norm[y])))
  }
} 
...
params <- c("psi.pred","int.p","int.psi","beta.site.type","beta.year","beta.year2")
...
params <- c("psi.pred[1, 1]","psi.pred[1, 2]","psi.pred[1, 3]","psi.pred[1, 4]","psi.pred[1, 5]","psi.pred[1, 6]","psi.pred[1, 7]","psi.pred[1, 8]","psi.pred[1, 9]","psi.pred[1, 10]","psi.pred[1, 11]","psi.pred[1, 12]",
            ...
            "int.p","int.psi","beta.site.type[2]","beta.year","beta.year2") 
...
#coefficients mean
results.mat[i,97:101] <- apply(out.all[,25:29], 2, mean)  
#coefficients sd 
results.mat[i,102:106] <- apply(out.all[,25:29], 2, sd)
#coefficients LCI 
results.mat[i,107:111] <- apply(out.all[,25:29], 2, function(x)quantile(x,probs=0.025)) 
#coefficients UCI 
results.mat[i,112:116] <- apply(out.all[,25:29], 2, function(x)quantile(x,probs=0.975)) 

#WAIC
results.mat[i,117] <- out$WAIC$WAIC
#R-hat 
results.mat[i,118] <- gelman.diag(out$samples[,c(2,3,4,5,6)],multivariate=TRUE)$mpsrf
...
colnames(results.mat) <- c(paste(params[1:24],".mean",sep=""),paste(params[1:24],".sd",sep=""),paste(params[1:24],".LCI",sep=""),paste(params[1:24],".UCI",sep=""),paste(params[25:29],".mean",sep=""),paste(params[25:29],".sd",sep=""),paste(params[25:29],".LCI",sep=""),paste(params[25:29],".UCI",sep=""),"WAIC","R.hat")

all.species <- apply(z.pred,c(2,3,4),sum)


write.csv(results.mat,"results/occ_model6_results.csv")
write.csv(all.species,"results/occ_model6_allspp.csv")
