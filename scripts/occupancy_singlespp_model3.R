...
for(s in 1:n.sites){
  for(y in 1:n.years){
    ...
    logit(psi[s,y]) <- int.psi + beta.year*year.norm[y]
  }
}
  
for(s in 1:2){
  for(y in 1:n.years){
    psi.pred[s,y] <- 1/(1+exp(-(int.psi + beta.year*year.norm[y])))
  }
} 
...
params <- c("psi.pred","int.p","int.psi","beta.year")
...
#coefficients mean
results.mat[i,97:99] <- apply(out.all[,25:27], 2, mean)  
#coefficients sd 
results.mat[i,100:102] <- apply(out.all[,25:27], 2, sd)
#coefficients LCI 
results.mat[i,103:105] <- apply(out.all[,25:27], 2, function(x)quantile(x,probs=0.025)) 
#coefficients UCI 
results.mat[i,106:108] <- apply(out.all[,25:27], 2, function(x)quantile(x,probs=0.975)) 

#WAIC
results.mat[i,109] <- out$WAIC$WAIC
#R-hat 
results.mat[i,110] <- gelman.diag(out$samples[,c(1:3)],multivariate=TRUE)$mpsrf
...
write.csv(results.mat,"results/occ_model3_results.csv")
write.csv(all.species,"results/occ_model3_allspp.csv")
