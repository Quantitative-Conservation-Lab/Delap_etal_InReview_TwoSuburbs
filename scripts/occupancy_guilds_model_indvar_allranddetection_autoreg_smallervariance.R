...
GUILD <- "exploiters"  #options are "avoiders", "adapters", "exploiters"
...
for(k in 1:n.species){
  p.species[k] ~ dnorm(0,sd = sd.p.species)
  psi.species[k] ~ dnorm(0,sd = sd.psi.species)
}  
#Priors 
int.p ~ dnorm(0,sd=1)
sd.p ~ dunif(0,1) 
...
params <- c("int.p","sd.p","sd.p.species","int.psi","beta.site.type","beta.year","beta.year2","sd.psi.species","autoreg.psi","w.sitetype","w.year","w.year2") 
