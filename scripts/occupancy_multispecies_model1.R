...
results.mat <- matrix(NA,nrow=Species,ncol = 106)
...
for(s in 1:n.sites){
  for(y in 1:n.years){
    for(k in 1:n.species){
      ...
      logit(psi[s,y,k]) <- int.psi + psi.ran.species[k]
    }#k species 
  }#y year
}#j site 
...
#coefficients mean
results.mat[i,97:98] <- apply(out.all[,25:26], 2, mean)  
#coefficients sd 
results.mat[i,99:100] <- apply(out.all[,25:26], 2, sd)
#coefficients LCI 
results.mat[i,101:102] <- apply(out.all[,25:26], 2, function(x)quantile(x,probs=0.025)) 
#coefficients UCI 
results.mat[i,103:104] <- apply(out.all[,25:26], 2, function(x)quantile(x,probs=0.975)) 

#WAIC
results.mat[i,105] <- out$WAIC$WAIC
#R-hat 
